...
let
  ...
  dmsStylixTheme = {
    ...
    light = if config.stylix.polarity == "dark" then dmsStylixInvertedTheme else dmsStylixBaseTheme;
    dark = if config.stylix.polarity == "dark" then dmsStylixBaseTheme else dmsStylixInvertedTheme;
  };
in
{
  imports = [
