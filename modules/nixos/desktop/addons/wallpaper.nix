# Wallpaper addon with DMS session strategy and user wallpaper collection links.
{
  config,
  lib,
  pkgs,
...
let
  cfg = config.custom.desktop.addons.wallpaper;
...
in
{
  options.custom.desktop.addons.wallpaper = {
    enable = lib.mkEnableOption "desktop wallpaper addon";
    ...
    assertions = [
      {
        ...
        message = "custom.desktop.addons.wallpaper.directory must be set when wallpaper addon is enabled.";
      }
      {
        assertion = wallpaperNames != [ ] || cfg.initialStrategy == "none";
        message = "custom.desktop.addons.wallpaper.directory does not contain any supported wallpaper image.";
      }
      {
        assertion =
          cfg.initialStrategy != "fixed" || (cfg.fixedFile != null && lib.elem cfg.fixedFile wallpaperNames);
        message = "custom.desktop.addons.wallpaper.fixedFile must name an image in the wallpaper directory.";
      }
    ];
