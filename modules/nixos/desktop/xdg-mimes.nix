...
let
  cfg = config.custom.desktop.xdg-mimes;
...
in
{
  options.custom.desktop.xdg-mimes = {
    enable = lib.mkOption {
      type = lib.types.bool;
      default = true;
      description = "Enable default XDG MIME associations for desktop users.";
    };

    manageUserDefaults = lib.mkOption {
      type = lib.types.bool;
      default = true;
      description = "Inject XDG MIME defaults into users managed by the system desktop profile.";
    };
  };
